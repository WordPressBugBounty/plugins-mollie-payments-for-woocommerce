...
class MollieSubscriptionGateway extends MolliePaymentGateway
{
     ...
     */
    public function __construct(PaymentMethodI $paymentMethod, PaymentService $paymentService, OrderInstructionsService $orderInstructionsService, MollieOrderService $mollieOrderService, Data $dataService, Logger $logger, NoticeInterface $notice, HttpResponse $httpResponse, Settings $settingsHelper, MollieObject $mollieObject, PaymentFactory $paymentFactory, string $pluginId, Api $apiHelper)
    {
        parent::__construct($paymentMethod, $paymentService, $orderInstructionsService, $mollieOrderService, $dataService, $logger, $notice, $httpResponse, $mollieObject, $paymentFactory, $pluginId);
        $this->apiHelper = $apiHelper;
        $this->subscriptionObject = new MollieSubscription($pluginId, $apiHelper, $settingsHelper, $dataService, $logger);
    ...
    {
        $this->isSubscriptionPayment = \true;
    ...
    public function update_subscription_status_for_direct_debit($renewal_order)
    {
        ...
        if (wcs_order_contains_renewal($renewal_order_id)) {
            ...
            $mollie_payment_id = $renewal_order->get_meta('_mollie_payment_id', $single = \true);
            $subscription_id = $renewal_order->get_meta('_subscription_renewal', $single = \true);
    ...
    public function scheduled_subscription_payment($renewal_total, WC_Order $renewal_order)
    {
        ...
        try {
            ...
            try {
                if (!empty($mandateId)) {
                    list($mandate, $data, $validMandate) = $this->usePreviousMandate($renewal_order_id, $customer_id, $mollieApiClient, $mandateId, $isRenewalMethodDirectDebit, $data, $validMandate);
                }
                if (!$validMandate) {
                    list($validMandate, $data) = $this->useAnyValidMandate($renewal_order_id, $customer_id, $mollieApiClient, $validMandate, $data, $renewalOrderMethod);
                }
            ...
            try {
                if ($validMandate) {
                    ...
                    if (property_exists($payment, 'mandateId') && $payment->mandateId !== null && $payment->mandateId !== $mandateId && !empty($subscriptionParentOrder)) {
    ...
    public function add_subscription_payment_meta($payment_meta, $subscription)
    {
        ...
        $payment_meta[$this->id] = ['post_meta' => ['_mollie_payment_id' => ['value' => $mollie_payment_id, 'label' => 'Mollie Payment ID'], '_mollie_payment_mode' => ['value' => $mollie_payment_mode, 'label' => 'Mollie Payment Mode'], '_mollie_customer_id' => ['value' => $mollie_customer_id, 'label' => 'Mollie Customer ID']]];
        return $payment_meta;
    }
    /**
    ...
    {
        add_filter($this->pluginId . '_is_subscription_payment', function ($isSubscription, $orderId) {
            if ($this->dataService->isWcSubscription($orderId)) {
                add_filter($this->pluginId . '_is_automatic_payment_disabled', static function ($filteredOption) {
     ...
     * @throws ApiException
     */
    protected function usePreviousMandate(int $renewal_order_id, $customer_id, \Mollie\Api\MollieApiClient $mollieApiClient, $mandateId, bool $isRenewalMethodDirectDebit, $data, bool $validMandate): array
    {
     ...
     * @throws ApiException
     */
    protected function useAnyValidMandate(int $renewal_order_id, $customer_id, \Mollie\Api\MollieApiClient $mollieApiClient, bool $validMandate, $data, $renewalOrderMethod): array
    {
